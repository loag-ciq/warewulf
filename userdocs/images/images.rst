...
a ``wwctl image shell`` or ``wwctl image exec``. If ``--build`` is specified,
``image_exit.sh`` is executed before (re)building the image. This is typically
used to remove cache or log files that may have been generated by the executed
